namespace Lab3
{
    public partial class Form1 : Form
    {
        ...
        {
            // Угол поворота в радианах (90 градусов)
            double angle = 90 * Math.PI / 180;

            // Матрица поворота (double для точности)
            ...
            // Временный массив для хранения повёрнутых координат (double)
            ...
            for (int i = 0; i < 5; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    ...
                    for (int k = 0; k < 3; k++)
                    {
                        temp[i, j] += kv[i, k] * rotateMatrix[k, j];
                    }
                }
            }
            ...
            // Отрисовка повёрнутой фигуры (фиолетовый цвет)
